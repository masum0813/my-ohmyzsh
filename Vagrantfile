...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  boxes.each do |opts|
    config.vm.define opts[:name] do |box|
      ...
      box.vm.provision "shell", inline: "cd /vagrant && ./provision/install-ansible.sh", privileged: false


      box.vm.provider "virtualbox" do |vb|
        # Naming for Virtualbox list
        vb.name = "my-ohmyzsh"
        vb.cpus = opts[:cpus]
      ...
      box.vm.provision "devops",     type:"shell", run: "never", inline: "cd /vagrant && ./provision/install-devops-tools.sh", privileged: false
